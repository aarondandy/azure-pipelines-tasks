...
function CreateProjectFiles(packageName: string, version: string): string[] {
    ...
    updateXmlFile(projectFilePath, (xml: any): any => {
        if (xml) {
            let xmlPackageSources = getOrCreateLastElement(xml, "ItemGroup");
            let xmlPropertyGroup = xmlPackageSources.c("PackageReference", {
                Include: packageName,
                Version: version
            });
        }
...
function GetExternalAuthInfoArray(inputKey: string, username: string): auth.ExternalAuthInfo[]
{
    ...
    endpointNames.forEach((endpointName: string) => {
        ...
        switch(scheme) {
            case "token":
                let token = externalAuth.parameters["AccessToken"];
...
function GetGitHubUser(endpointId: string): Promise<string> {
    ...
    if (scheme !== "token") {
        console.log("Scheme not correct");
